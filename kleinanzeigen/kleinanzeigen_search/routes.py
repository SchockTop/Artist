...
def route_via_osrm(client: HttpClient, points: list[geo.Point], osrm_url: str = OSRM_URL) -> tuple[list[geo.Point], float, float]:
    coords = ";".join(f"{lon:.6f},{lat:.6f}" for lat, lon in points)
    payload = client.get_json(osrm_url.format(coords=coords), use_cache=True)
    if payload.get("code") != "Ok" or not payload.get("routes"):
        raise RuntimeError(f"routing failed: {payload.get('message') or payload.get('code')}")
    route = payload["routes"][0]
    return (
        geo.decode_polyline(route["geometry"]),
...
def build_route(
    ...
    if polyline:
        ...
        return Route(geo.simplify(points, simplify_km), source="polyline")

    if gpx_path:
        points = load_gpx(gpx_path)
        if len(points) < 2:
            raise ValueError(f"{gpx_path} contains fewer than two track points")
        return Route(geo.simplify(points, simplify_km), source="gpx")
    ...
    try:
        points, distance_km, duration_min = route_via_osrm(client, resolved, osrm_url)
        return Route(geo.simplify(points, simplify_km), labels, distance_km, duration_min, "osrm")
        ...
        return Route(points, labels, source="straight-line (routing unavailable)")
